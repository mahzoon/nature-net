...
namespace nature_net.user_controls
{
    public class BalloonDecorator : Decorator
    {
        ...
        public static readonly DependencyProperty PointerVerticalOffsetProperty =
            DependencyProperty.Register("PointerVerticalOffset", typeof(double), typeof(BalloonDecorator));
        ...
        public double PointerVerticalOffset
        {
            get { return (double)GetValue(PointerVerticalOffsetProperty); }
            set { SetValue(PointerVerticalOffsetProperty, value); }
        }
        ...
        private StreamGeometry CreateBalloonGeometry(Rect rect)
        {
            ...
            using (StreamGeometryContext ctx = geometry.Open())
            {
                ...
                if ((pointerLength > 0) && (CornerPosition == "Right"))
                {
                    ctx.LineTo(new Point(points[3].X, points[3].Y - PointerVerticalOffset), true, false);
                    ctx.LineTo(new Point(rect.Width, points[3].Y - PointerVerticalOffset + radius), true, false);
                    ctx.LineTo(new Point(rect.Width - pointerLength, points[3].Y - PointerVerticalOffset + radius + radius), true, false);
                }
                ...
                if ((pointerLength > 0) && (CornerPosition == "Left"))
                {
                    ctx.LineTo(new Point(points[7].X, points[7].Y + PointerVerticalOffset + radius + radius), true, false);
                    ctx.LineTo(new Point(rect.BottomLeft.X, points[7].Y + PointerVerticalOffset + radius), true, false);
                    ctx.LineTo(new Point(points[7].X, points[7].Y + PointerVerticalOffset), true, false);
                }
